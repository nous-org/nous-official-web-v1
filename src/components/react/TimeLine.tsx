...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className="w-full bg-gradient-to-b from-primary-blue via-outline/10 to-primary-blue/40 font-sans md:px-10"
      ref={containerRef}
    >
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-primary-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-outline/25 border border-outline/60 p-2" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-medium text-neutral-400">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full appear-subtle">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-medium text-neutral-400">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0  w-[2px] bg-gradient-to-t from-outline via-gradient-end to-transparent from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
